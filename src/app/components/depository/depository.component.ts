...
import { DepositoryService, PrinterService, TransactionLogService, DepositService } from '../../services';
import { DepositRequest, PrintRequest, DepositoryCountResponse } from 'src/app/payload';
...
export class DepositoryComponent implements OnInit, OnDestroy {
  ...
  public printReceipt(value: string): void {
    ...
    const request: PrintRequest = {
      companyId: this.commAreaService.companyId,
      unitId: this.commAreaService.unitId,
      email: this.commAreaService.user.email,
      data: "*** COMPROVANTE ***"
    };

    this._subscribe = this.printerService.print(request).subscribe(
      (response) => {
        setTimeout(() => {
          this.goBack();
        }, 3000);
